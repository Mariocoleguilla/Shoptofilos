...
?>
    ...
    <head>
        <title>Administración de ventas | Shoptófilos</title>
    ...
    <body>
        ...
        <main>
            ...
            <section class="container">
                <form id="form1" role="form" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="POST" autocomplete="off">
                    ...
                    <br/>
                    <input type="number" id="id_venta" name="id_venta" placeholder="ID Venta" required="required" />
                    <br/>
                    <br/>
        ...
        if (!empty($_POST)) {
            ...
            if (isset($_POST['edit'])) { //Comprobación del segundo formulario
                ...
                if (strlen($_POST['date']) < 1) {
                    ...
                    if (!preg_match($dateRegExp, $_POST['date'])) {
                        ...
                        $sql = "UPDATE juegos_venta SET fecha = \"" . $_POST['date'] . "\" WHERE id_venta = " . $_POST['id_venta2'];
            ...
            if (isset($_POST['delete'])) {
                ...
                $sql = "UPDATE ejemplares SET estado = 'libre' WHERE id_ejemplar = (SELECT id_ejemplar FROM juegos_venta WHERE id_venta = " . $_POST['id_venta2'] . ");";

                if ($conn->query($sql) === true) {

                    $sql = "DELETE FROM juegos_venta WHERE id_venta = " . $_POST['id_venta2'];
            ...
            if (isset($_POST['query'])) {
                $sql = "SELECT j.id_juego, j.titulo, j.plataforma, j.estudio, j.genero, v.fecha, v.dni, v.id_venta, v.id_ejemplar
                    FROM juegos j, juegos_venta v, ejemplares e
                    WHERE j.id_juego = e.id_juego
                    AND e.id_ejemplar = v.id_ejemplar
                    AND v.id_venta = " . $_POST['id_venta'] . "
                    ORDER BY id_venta";
                ...
                ?>
                    ...
                    <form id='form2' role='form' action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method='POST' autocomplete='off'>
                        <br/>
                        <label for='id_venta2'>ID Venta</label>
                        <br/>
                        <input type='text' id='id_venta2' class="redInput" name='id_venta2' value="<?php echo $row['id_venta']; ?>" placeholder='ID Venta' required="required"
                            readonly="readonly" />
                        <br/>
                        <br/>
                        <label for='id_juego'>ID Juego</label>
                        <br/>
                        <input type='text' id='id_juego' class="redInput" name='id_juego' value="<?php echo $row['id_juego']; ?>" placeholder='ID Juego' required="required"
                            readonly="readonly" />
                        <br/>
                        <br/>
                        <label for='titulo'>Titulo</label>
                        <br/>
                        <input type='text' id='title' class="redInput" name='title' value="<?php echo $row['titulo']; ?>" placeholder='Titulo' required="required"
                            readonly="readonly" />
                        <br/>
                        <br/>
                        <label for='plataforma'>Plataforma</label>
                        <br/>
                        <input type='text' id='platform' class="redInput" name='platform' value="<?php echo $row['plataforma']; ?>" placeholder='Plataforma' required="required"
                            readonly="readonly" />
                        <br/>
                        <br/>
                        <label for='estudio'>Estudio</label>
                        <br/>
                        <input type='text' id='studio' class="redInput" name='studio' value="<?php echo $row['estudio']; ?>" placeholder='Estudio' required="required"
                            readonly="readonly" />
                        <br/>
                        <br/>
                        <label for='genero'>Género</label>
                        <br/>
                        <input type='text' id='genre' class="redInput" name='genre' value="<?php echo $row['genero']; ?>" placeholder='Género' required="required"
                            readonly="readonly" />
                        <br/>
                        <br/>
                        ...
                        <br/>
                        <input type='text' id='id_ejemplar' class="redInput" name='id_ejemplar' value="<?php echo $row['id_ejemplar']; ?>" placeholder='ID Ejemplar'
                            required="required" readonly="readonly" />
                        <br/>
                        <br/>
                        <button type='submit' id='edit' value='edit' name='edit'>Efectuar Cambios</button>
                        <button type='submit' id='delete' value='delete' name='delete'>Eliminar Venta</button>
                    </form>

                    <?php
                    //HTML - PHP
                } else {
                    $warnings = "<p class=\"advice warning\">No existe ninguna venta con ese ID</p>";
                }
            }
        ...
        if ($result->num_rows > 0) {
            ...
            $warnings = "<p class=\"advice warning\">No hay ninguna venta</p>";
        }
